...
def update_token_set(token_map, token_set, token, operator, state):
    ...
    if operator == 'and':
        ...
        token_set = [i for i in token_set] + [i for i in lst]
    return token_set


if __name__ == "__main__":
    token_map = read_token_map('token_map.txt')
    query = input()
    ...
    while len(tokens) > 0:
        ...
        if token == 'not':
            state = token
            token = tokens.pop(0)
            token_set = update_token_set(token_map, token_set, token, operator, state)
